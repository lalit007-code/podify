...
export const getPodcastBySearch = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.search === "") {
      // If empty, return all podcasts ordered by date
      return await ctx.db.query("podcasts").order("desc").collect();
    }
    ...
    // If no results for author, try searching by title
    const titleSearch = await ctx.db
      .query("podcasts")
      .withSearchIndex("search_title", (q) =>
        q.search("podcastTitle", args.search)
    ...
    // If no results for title, try searching by body (description or title)
    ...
    // If no results found, fallback to returning the most recent podcasts
    return await ctx.db.query("podcasts").order("desc").collect();
  },
});
